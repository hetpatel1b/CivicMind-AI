        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          <AnimatePresence>
            {error && (
              ...
              >
                <div className="p-3 bg-red-500/10 text-red-400 text-sm font-medium rounded-xl border border-red-500/20 flex items-center gap-2 mb-2">
          ...
          <div className="space-y-4">
            ...
            <div className="relative group">
              ...
              <input
                ...
                type="text"
                required
                className="block w-full pl-11 pr-4 py-3 bg-gray-900/50 border border-gray-700/50 rounded-xl text-white placeholder-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all peer"
                placeholder="Jane Doe"
              />
              <label 
                htmlFor="fullName" 
                className="absolute left-11 -top-2.5 bg-[#0c1222] px-1 text-xs font-medium text-gray-400 peer-focus:text-indigo-400 transition-all peer-placeholder-shown:text-sm peer-placeholder-shown:top-3 peer-focus:-top-2.5 peer-focus:text-xs pointer-events-none"
              >
                Full Name
              </label>
            </div>
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="block w-full pl-11 pr-12 py-3 bg-gray-900/50 border border-gray-700/50 rounded-xl text-white placeholder-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all peer"
                placeholder="••••••••"
              />
              <label 
                htmlFor="password" 
            ...
            {password.length > 0 && (
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="flex-1 flex gap-1 h-1.5 rounded-full overflow-hidden bg-gray-800">
                    {[1, 2, 3, 4].map((level) => (
                      <div 
                        key={level} 
                        className={`flex-1 transition-all duration-500 ${level <= strengthScore ? strengthColor : 'bg-transparent'}`} 
                      />
                    ))}
                  </div>
                  <span className={`text-xs font-bold ${strengthScore <= 1 ? 'text-red-400' : strengthScore <= 3 ? 'text-yellow-400' : 'text-green-400'}`}>
                    {strengthLabel}
                  </span>
                </div>
                
                <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs">
                  <ReqItem fulfilled={reqLength} label="8+ characters" />
                  <ReqItem fulfilled={reqUpper} label="Uppercase letter" />
                  <ReqItem fulfilled={reqNumber} label="Number" />
                  <ReqItem fulfilled={reqSpecial} label="Special character" />
                </div>
              </motion.div>
            )}
          ...
          <div className="pt-2 pb-1">
            <label className="flex items-start gap-3 cursor-pointer group">
              <div className="relative flex items-center pt-0.5">
                <input required type="checkbox" className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-gray-900 transition-colors cursor-pointer" />
              </div>
              <span className="text-xs text-gray-400 group-hover:text-gray-300 transition-colors leading-relaxed">
                By creating an account, you agree to our{' '}
                <a href="#" className="font-medium text-indigo-400 hover:text-indigo-300 hover:underline">Terms of Service</a> and{' '}
                <a href="#" className="font-medium text-indigo-400 hover:text-indigo-300 hover:underline">Privacy Policy</a>.
              </span>
            </label>
          </div>
...
function ReqItem({ fulfilled, label }: { fulfilled: boolean, label: string }) {
  return (
    <div className={`flex items-center gap-1.5 transition-colors ${fulfilled ? 'text-green-400' : 'text-gray-500'}`}>
      {fulfilled ? <Check className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5" />}
      {label}
    </div>
  )
}
